...
class WrappedUpload extends React.Component{
  ...
  componentWillMount() {
    ...
    if (component_attachment && componentType === 'File') {
      ...
      let fileInfo = { name, ext, size: bytesToSize(component_attachment.filesize) }
  ...
  _uploadImage = (e, context, isFile) => {
    ...
    if (isFile || e.target.files && e.target.files[0]) {
      ...
      let fileInfo = { name: file.name.split('.')[0], ext: extension, size: bytesToSize(file.size)  }
      this.setState({ fileInfo })
    ...
    return(
      ...
      > 
        {                                               // UI to show for file uploads
          fileInfo && file
          &&
          <a href={component_attachment && component_attachment.url} style={{ textDecoration: 'none' }} target="_blank">
            <div className="file-data-section">
              <div className="file-data-icon">
                <img src={this.getFileIcon(fileInfo.ext)} alt="Alt image" className="file-data-icon-img"/>
              </div>
