...
export const getVideoUrl = (url) => {
  let videoid = url.replace(
    /(https{0,1}:\/\/){0,1}(www\.){0,1}((youtube.com\/{0,1}(watch\?v=){0,1})|(vimeo.com\/{0,1}))/g,
    ""
  );
...
export function compareAndDiff(newState, newData) {
  ...
  while (i < newState.length) {
    if (newData[j]) {
      if (
        newState[i].id === newData[j].id ||
        newState[i].content === newData[j].content ||
        (newData[j].component_attachment &&
          newState[i].component_attachment &&
          newState[i].component_attachment.filename ===
            newData[j].component_attachment.filename)
      ) {
...
export const convertObjectToText = (root) => {
  ...
  return `<${root.tagName}${
    root.tagName === "a" ? ` href="${root.attributes.href}"` : ""
  }>${root.childNodes.map((node) => convertObjectToText(node)).join("")}</${
    root.tagName
  }>`;
};
      ...
      formattedOptions.push({
        ...
        isSelected:
          ...
          selectedOptions.some(
            (e) =>
              e &&
              (Number(item.id) === e.id || Number(item.id) === e.constant_id)
          ),
      });
    });
