...
export const bulkCreate = (parsedData, e) => {
  return (dispatch, getState) => {
            ...
            //Handle block component => truncate components which are not supported.
              ...
              const getImage = (src) => {
                return !src.includes("base64")
                  ? { filename: "attachements", url: src }
                  : { filename: "attachements", content: src };
              };
              newData.push({
                content: sanitizeHtml(
                  node.tagName === "li"
                    ? node.childNodes[0].rawText
                    : node.rawText
                ),
                component_attachment:
                  node.tagName === "img" ? getImage(node.attributes.src) : null,
    ...
    if (
      !(
        newData.length === 1 &&
        TEXT_COMPONENT.includes(newData[0].componentType)
      )
    ) {
      e.preventDefault();
      dispatch({
        type: BULK_ADD_COMPONENT,
        data: { newData, focusedElemId: getState().currentElem.elemId },
      });
    }
  };
};
      ...
      newCom = {
        content: data.content
          ? typeof content === "string"
            ...
            : data.content
          : "", //Sanitizing only strings, exception for file objects
        position: position + 1,
        componentType: componentType,
        id: data.newId,
        component_attachment: data.component_attachment
          ? { ...data.component_attachment }
          : null,
      };
      temp.push(newCom);
      position += 2;
...
function updateComponentPos(state, { newIndex, oldIndex }) {
  ...
  emitUpdate(
    { id: state.componentData[oldIndex].id, position: newIndex + 1 },
    "updatePos"
  );
  return {
    ...state,
